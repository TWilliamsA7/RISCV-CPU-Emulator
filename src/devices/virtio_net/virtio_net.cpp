...
// virtio_net_hdr for non-MRG_RXBUF + VIRTIO_F_VERSION_1 (12 bytes)
struct VirtioNetHdr {
    uint8_t  flags;
    uint8_t  gso_type;
    uint16_t hdr_len;
    ...
    uint16_t num_buffers; // required when VIRTIO_F_VERSION_1 negotiated
};
...
VirtioNet::~VirtioNet() {
    // Backend cleanup is handled by virtio_net_slirp.cpp via backend_
    // The poll thread is stopped there.
}
...
uint32_t VirtioNet::read32(uint32_t offset) {
    ...
    switch (offset) {
        ...
        case VIRTIO_CONFIG + 0:    // MAC bytes 0-3
            return (uint32_t)mac_[0]
                 | ((uint32_t)mac_[1] << 8)
                 | ((uint32_t)mac_[2] << 16)
                 | ((uint32_t)mac_[3] << 24);
        case VIRTIO_CONFIG + 4:    // MAC bytes 4-5 + link status
...
// ── MMIO read (8-bit) — for config space byte accesses ───────────────────────
uint8_t VirtioNet::read8(uint32_t offset) {
    if (offset >= 0x100 && offset < 0x106)
        return mac_[offset - 0x100];
    if (offset == 0x106) return 1; // link status low byte = LINK_UP
    if (offset == 0x107) return 0;
    // Fall back to 32-bit for other registers
...
void VirtioNet::rx_inject(const uint8_t* frame, uint32_t len) {
    ...
    while (true) {
        ...
        if (first) {
            VirtioNetHdr hdr{};
            hdr.num_buffers = 1; // spec: MUST be 1 without MRG_RXBUF
